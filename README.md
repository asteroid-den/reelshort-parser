...
- All IO operations (HTTP requests, files reading/writing) are async, series are parsed concurrently
- Process consists of two phases: firstly, we parse all series short data from /tags/XYZ tabs; secondly, we parse full data from specific /movie/XYZ pages for each series
